...
function GenerateQuestionsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedData = {
      ...
      question_type: ["MCQ", "descriptive"], // Hardcoded question types
      difficulty_level: `${formData.difficultyLevel} on 10`,
      total_questions: formData.numberOfQuestions,
    ...
    const input = JSON.stringify(formattedData);

    try {
      const response = await fetch(
        "https://himmaannsshhuu-langflow.hf.space/api/v1/run/2b629d5c-fc8a-4093-9178-4ad3037f8f9a?stream=false",
        {
          method: "POST",
          headers: {
      ...
      console.log("Data Response:", data?.['outputs']?.[0]?.["outputs"]?.[0]?.["artifacts"]?.["message"],);
