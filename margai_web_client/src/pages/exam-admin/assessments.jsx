...
function AssignmentsPage() {
  const { t } = useTranslation("exam-admin-assessments-page");
    ...
    <>
      ...
      <div className="ml-[16.975rem] bg-[#E2EBFF] h-full pb-8">
        <header className="flex items-center justify-between px-10 py-6">
          <h1 className="text-4xl font-bold text-[#1B2E59]">{t("title")}</h1>
        ...
        <div className="px-10 pt-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[#1B2E59] text-2xl font-bold">
              {t("active_section_label")}:{" "}
            </h2>
...
function ExamCard({ assessment }) {
  ...
  const status = {
    active: new Date(startDate) < new Date() && new Date(endDate) > new Date(),
    scheduled: new Date(startDate) > new Date(),
    ended: new Date(endDate) < new Date(),
  };
  ...
  return (
    <div className="bg-white rounded-3xl shadow-md px-6 py-4 flex flex-col ">
      ...
      <h3 className="text-[0.9rem] font-semibold text-[#8C1D18] mb-4">
        {status.ended ? "Finished:" : "Deadline:"} {ordinalDate(endDate, true)}
      </h3>
      ...
      <button className="bg-[#1B2E59] rounded-3xl mx-6 px-3 py-2 text-white font-bold mb-2">
        {status.active ? "Details" : status.scheduled ? "View" : "Monitor"}
      </button>
